...
const resolvers = {
  ...
  Author: {
    bookCount: (root) => {
      const authBooks = books.filter((book) => book.author === root.name);
  ...
  Mutation: {
    addBook: (root, args) => {
      // katsotaan löytyykö author
      const existingAuthor = authors.some(
        (author) => author.name === args.author,
      );
      // luodan uusi kirja jos authoria ei löydy
      if (!existingAuthor) {
        const newAuthor = {
          name: args.author,
          id: uuid(),
    ...
    editAuthor: (root, args) => {
      const queriedAuthor = authors.find((author) => author.name === args.name);

      if (!queriedAuthor) {
        return null;
      }
      
      const updatedAuthor = { ...queriedAuthor, born: args.setBornTo };
